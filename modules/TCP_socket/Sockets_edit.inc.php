  ...
  if ($this->tab=='data') {
   ...
   if ($this->mode=='update') {
	...
	if ($title_new) {
	 ...
	 $new_id=SQLInsert('Channels',$prop);
	}
   }
   global $delete_id;
   if ($delete_id) {
    SQLExec("DELETE FROM Channels WHERE ID='".(int)$delete_id."'");
   }
   $properties=SQLSelect("SELECT * FROM Channels WHERE DEVICE_ID='".$rec['ID']."' ORDER BY ID");
   ...
   for($i=0;$i<$total;$i++) {
    ...
    if ($this->mode=='update') {
      ...
      SQLUpdate('Channels', $properties[$i]);
